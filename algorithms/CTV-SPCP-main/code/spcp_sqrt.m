function [A_hat,E_hat,iter] = spcp_sqrt(D,lambda, tol, maxIter)
...
while ~converged       
    ...
    temp  = D-A_hat-E_hat+(1/mu)*Y;
    normTemp = norm(temp,'fro');
    N_hat = max(normTemp-beta/mu,0)*temp/normTemp;
%     sigma = std(N_hat(:))
%     beta  = 1/(2*sqrt(m+n)*sigma);
%     
