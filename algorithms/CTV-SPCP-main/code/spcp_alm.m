function [A_hat,E_hat,iter] = spcp_alm(D,sigma,lambda, tol, maxIter)
...
[m,n] = size(D);
eps = 1e-6;
beta = 1/((sqrt(m)+sqrt(n))*sigma + eps);
if nargin < 3
    lambda = 1 / sqrt(m);
end
...
while ~converged       
    ...
    if choosvd(n, sv) == 1
        [U,S,V] = lansvd(D - E_hat - N_hat + (1/mu)*Y, sv, 'L');
    else
    ...
    N_hat = (Y+mu*(D-A_hat-E_hat))/(beta+mu);% Gaussian noise
    
%     sigma = std(N_hat(:));
%     beta = 1/(2*(sqrt(m)+sqrt(n))*sigma + eps);
%     
